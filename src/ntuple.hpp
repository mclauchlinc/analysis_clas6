...
private:
	...
	Int_t _evntb[NUM_THREADS][2]; //The #event for the given file
	Int_t _apartb[NUM_THREADS][2];//The particle in each event
	Int_t _bpartb[NUM_THREADS][2];  
	Float_t _pxb[NUM_THREADS][2][6];
	Float_t _pyb[NUM_THREADS][2][6]; 
	Float_t _pzb[NUM_THREADS][2][6];
	Float_t _p0b[NUM_THREADS][2][6];//Energy of the particle 
	Int_t _pidb[NUM_THREADS][2][6];//particle ID 
	Int_t _helb[NUM_THREADS][2]; //helicity
	Int_t _topb[NUM_THREADS][2]; //Topology {pmiss,pipmiss,pimmiss,zero} -> {1,2,3,4}
	Int_t _fc_totb[NUM_THREADS][2];
	Float_t _MMb[NUM_THREADS][2][3];//{p/pip,p/pim,pip/pim}
	Float_t _thetab[NUM_THREADS][2][3];//{p,pip,pim}
	Float_t _alphab[NUM_THREADS][2][3];//[{pim,p},{pp,pip}],[{p,pp},{pip,pim}],[{pip,p},{pp,pim}]
	Int_t _run_typeb[NUM_THREADS][2];//{1,2,3,4}->{e16,e1f,e16sim,e1f sim} 
	Float_t _weightb[NUM_THREADS][2]; 
	Int_t _COMb[NUM_THREADS][2];
	/*
	...
	*/
	...
	TTree* _athrown_tree[NUM_THREADS];//Trees for the various threads 
	//TList* _the_forest;
	//TList* _thr_forest;
	TTree* _new_tree1;
	TTree* _new_tree2;
...
public:
	forest(int is_alive, bool sim_ = false);//-1 thread_id gives the whole tree, but the others give pieces of trees
	//~forest();
