...
void Particle::Fill_Particle(std::shared_ptr<Branches> data_, int par_idx_, int set_, bool sim_, float weight_, bool thrown_){
	float m = -99; 
	_idx = par_idx_;
	_set = set_;
	_sim = sim_;
	...
	if(_thrown){
		...
		if(data_->Branches::dc(_idx) > 0 && data_->Branches::sc(_idx) > 0 ){//These are things necessary for all events
			...
			_dtheta[1] = detect::sc_theta(data_,_idx);
			_dphi[1] = detect::sc_phi(data_,_idx);
		...
		if(!_sim && data_->Branches::cc(_idx) > 0){
			...
			if(_idx == 0){
				...
				_dtheta[0] = detect::cc_theta(data_,_idx);
				_dphi[0] = detect::cc_phi(data_,_idx);
			}
		...
		if(data_->Branches::ec(_idx)>0){
			...
			_dtheta[2] = detect::ec_theta(data_,_idx);
			_dphi[2] = detect::ec_phi(data_,_idx);
		}
	} 
}
...
void Particle::PID(std::shared_ptr<Branches> data_, std::shared_ptr<Environment> envi_, std::shared_ptr<Histogram> hist_, float W_){
	...
	if(_thrown){
		switch(data_->Branches::mcid(_idx)){
			case ELECTRON:
				_pid[0] = true;
				_ided = true;
				hist_->Histogram::WQ2_Fill(envi_, 0, 0, W_, physics::Qsquared(envi_->Environment::was_data_set(), data_, _thrown),1.0, 1);
			break;
			case PROTON:
				_pid[1] = true;
				_ided = true;
	...
	for(int j = 0; j<4; j++){
		for(int i = 0; i<3; i++){
			...
			if(!isnan(_dtheta[i]) && !isnan(_dphi[i]) && _sanity_pass[j]){
				hist_->Fid_Det_Fill(envi_,j,_dtheta[i],_dphi[i],physics::get_sector(_phi),i);
			}	
		}
	}
}
