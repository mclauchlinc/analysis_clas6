...
std::vector<std::string> read_file_list(std::string path, int thread_num) //Choose a vector of strings. Vector because adding things onto the end works fine. Also don't know how to use a list
{
...
void loadChain(TChain* c, std::string file, int thread_id, int max=-1)
{
