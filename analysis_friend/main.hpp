...
size_t run(std::shared_ptr<TChain> _chain, std::shared_ptr<Histogram> _hists, std::shared_ptr<forest> a_forest, std::shared_ptr<forest> thr_forest, int thread_id, int run_type, int plate_, std::shared_ptr<Environment> _envi){//, int &num_ppip){
	...
	for(size_t curr_event = 0; curr_event < num_of_events; curr_event++){
		...
		auto analysis = std::make_shared<Analysis>(data,_hists,_envi,run_type, a_forest, thr_forest, thread_id);
