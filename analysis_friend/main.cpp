...
int main(int argc, char **argv){
	...
	*/
	...
	thrown_forest->foest::mkfile(output_name,true);//Making the Tree File for thrown values
	std::future<bool> fut;
	...
	for(int i = 0; i<NUM_THREADS; i++){
		...
		threads[i] = std::async(run_files, infilenames.at(i),filepath_map[comp], hists, a_good_forest, thrown_forest, i, filetype_map[comp], file_num, _case, plate_stat, envi);//, num_mixed_p_pip[i]);
		//run_files(infilenames.at(i), filepath_map[argv[1]], hists, a_good_forest, i, data_set, file_num, _case);
		

		 //This is running the analysis 
	}
	for(int j = 0; j<NUM_THREADS; j++){//Wait until all the threads are done before proceeding
		threads[j].wait(); 
	}
	if(hel_runs){
	...
	if(data_set>2){
		thrown_forest->forest::Grow_Forest();
		thrown_forest->forest::Write();
	}
