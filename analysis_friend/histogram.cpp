		...
		while(cart.GetNextCombination()){
			if(_envi->Environment::was_sim()){
				if(cart[0]==10 && cart[1]!=0){
					sprintf(hname,"W_Q2_%s_%s_%s_%s",eid_cut[cart[0]],topologies[cart[1]],thrown[cart[2]],w_stat[cart[3]]); //constants.h and otherwise writing the specific cut to the right plot
