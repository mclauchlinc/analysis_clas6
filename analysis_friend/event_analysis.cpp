...
Analysis::Analysis(std::shared_ptr<Branches> data_, std::shared_ptr<Histogram> hist_, std::shared_ptr<Environment> envi_, int run_type_,std::shared_ptr<forest> a_forest_,std::shared_ptr<forest> thr_forest_, int thread_id_){
	switch(run_type_){
		case 1:
			_set = 1;
			_sim = false;
	...
	if(_sim){//Thrown analysis
		...
		//Only really need to use exclusive topology because we just want to line the particles up properly
		for(int k = 0; k< 4; k++){
			if(tPart[k].Particle::Is_Elec()){
				for(int l = 0; l < 4; l++){
					if(l!=k && tPart[l].Particle::Is_Pro()){//
						for(int n = 0; n< 4; n++){
							if(n!=l && n!=k && tPart[n].Particle::Is_Pip()){//Pim && Zero Miss
		...
		for(int x = 0; x<4; x++){
			tEvent[x].Event::Assign_Weight(_pweight);
			thr_forest_->forest::Fill_Thread_Tree(tEvent[x],x,thread_id_);
		}
